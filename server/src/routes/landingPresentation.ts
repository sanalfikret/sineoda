...
router.put('/',requireAdmin,(req,res)=>{
 ...
 const v=value as {count?:number;width?:string;align?:string;titleTr?:string;titleEn?:string;bodyTr?:string;bodyEn?:string;removed?:boolean;size?:string;cardSize?:string;columns?:number;mobile?:string}
 ...
 if (['titleTr','titleEn','bodyTr','bodyEn'].some(field=>{const val=(v as Record<string,unknown>)[field];return val!==undefined&&(typeof val!=='string'||val.length>20000)}) || (v.size!==undefined&&!['compact','normal','large'].includes(v.size)) || (v.cardSize!==undefined&&!['normal','large'].includes(v.cardSize)) || (v.columns!==undefined&&![3,4].includes(v.columns)) || (v.mobile!==undefined&&!['single','double'].includes(v.mobile)) || (v.removed!==undefined&&typeof v.removed!=='boolean')){res.sendStatus(400);return}
 result[key]={count:v.count,width:v.width,align:v.align,...(v.size?{size:v.size}:{}),...(v.cardSize?{cardSize:v.cardSize}:{}),...(v.columns?{columns:v.columns}:{}),...(v.mobile?{mobile:v.mobile}:{}),...(v.removed!==undefined?{removed:v.removed}:{}),...Object.fromEntries(['titleTr','titleEn','bodyTr','bodyEn'].filter(field=>(v as Record<string,unknown>)[field]!==undefined).map(field=>[field,(v as Record<string,unknown>)[field]]))}
 }
