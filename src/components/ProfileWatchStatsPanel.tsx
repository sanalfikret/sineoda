...
export function ProfileWatchStatsPanel({ profileId, profileName }: ProfileWatchStatsPanelProps) {
  ...
  if (loading) {
    return <p className="text-sm text-sineoda-muted">İzleme istatistikleri yükleniyor…</p>
  }
    ...
    <div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        <div className="rounded-xl border border-sineoda-gold/20 bg-sineoda-gold/5 p-4">
          <p className="text-xs uppercase tracking-wide text-sineoda-muted">Toplam izleme</p>
          <p className="mt-1 text-2xl font-semibold text-sineoda-gold">
            {formatWatchHours(stats.totalSeconds)}
          </p>
          <p className="mt-1 text-sm text-sineoda-muted">{stats.totalTitles} içerik</p>
        </div>
        {stats.byCategory.map((entry) => (
          <div key={entry.key} className="rounded-xl border border-white/10 bg-[#0d0f14] p-4">
            <p className="text-xs uppercase tracking-wide text-sineoda-muted">{entry.label}</p>
            <p className="mt-1 text-xl font-semibold">{formatWatchHours(entry.totalSeconds)}</p>
            <p className="mt-1 text-sm text-sineoda-muted">{entry.titlesWatched} içerik</p>
          </div>
        ))}
      </div>
