...
export function SearchModal({ onSelect, kidsSafe = false }: SearchModalProps) {
  ...
  const results = useMemo(() => {
    const found = searchContent(visibleCatalog, filters).filter(
      (item) => item.program !== 'student_cinema' && item.program !== 'shooting_notes',
    )
  ...
  return (
    <div
      className="safe-top safe-bottom fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-plooy-bg/95 backdrop-blur-md"
      role="dialog"
      aria-modal="true"
